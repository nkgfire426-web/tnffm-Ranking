...
function ok_(extra) { return json_(Object.assign({ ok: true }, extra || {})); }
function error_(message) { return json_({ ok: false, message: String(message || 'Unknown Google Sheets error.') }); }

function doGet(e) {
  try { return ok_(readAll_()); }
  catch (err) { console.error(err); return error_(err && err.message ? err.message : err); }
}
...
function doPost(e) {
  ...
  try {
    ...
    sections.forEach(function(pair) {
      ...
      writeSection_(pair[1], payload[key]);
      changed++;
    });
    if (!changed) return error_('No supported data section was supplied.');
    SpreadsheetApp.flush();
    return ok_({ savedSections: changed, savedAt: new Date().toISOString(), data: readAll_() });
  } catch (err) {
    console.error(err);
...
function cleanHeader_(value) { return String(value == null ? '' : value).trim(); }
...
function readSection_(name) {
  ...
  return values.slice(1).filter(function(row) {
    ...
    const item = {};
    headers.forEach(function(header, index) { if (header) item[header] = decodeCell_(row[index]); });
...
function normalizeReadObject_(input) {
  ...
  Object.keys(input).forEach(function(key) {
    ...
    if (canonical && !Object.prototype.hasOwnProperty.call(out, canonical)) out[canonical] = input[key];
  });
  return out;
}
...
function canonicalKey_(key) {
  const map = {
    'Team': 'teamName', 'Team Name': 'teamName', 'team': 'teamName', 'Slug': 'slug', 'Team ID': 'teamId', 'TeamId': 'teamId',
    'Rank': 'rank', 'PreviousRank': 'previousRank', 'Previous Rank': 'previousRank', 'CommunityPoints': 'communityPoints',
    'Community Score': 'communityScore', 'CommunityScore': 'communityScore', 'Badge': 'badge', 'Logo URL': 'logoUrl', 'LogoURL': 'logoUrl',
    'Banner URL': 'bannerUrl', 'BannerURL': 'bannerUrl', 'Kills': 'kills', 'Booyahs': 'booyahs', 'MatchesPlayed': 'matchesPlayed',
    'Matches Played': 'matchesPlayed', 'Championships': 'championships', 'RunnerUp': 'runnerUp', 'Runner-Up': 'runnerUp',
    'SecondRunnerUp': 'secondRunnerUp', '2nd Runner-Up': 'secondRunnerUp', 'Top5Finishes': 'top5Finishes', 'Top 5 Finishes': 'top5Finishes',
    'FinalistFinishes': 'finalistFinishes', 'OfficialMatchFinalists': 'officialMatchFinalists', 'EventsPlayed': 'eventsPlayed',
    'GrandFinals': 'grandFinals', 'PositionPoints': 'positionPoints', 'TotalPoints': 'totalPoints', 'Players': 'players', 'Status': 'status',
    'RegistrationStatus': 'registrationStatus', 'Description': 'description', 'Mobile Number': 'mobileNumber', 'LastUpdated': 'lastUpdated',
    'Last Updated': 'lastUpdated', 'WinRate': 'winRate', 'KillRatio': 'killRatio', 'BooyahRatio': 'booyahRatio', 'Event ID': 'eventId',
    'EventId': 'eventId', 'Name': 'name', 'Event Name': 'name', 'Organizer': 'organizer', 'Teams': 'teams', 'Team Count': 'teams',
    'Prize': 'prize', 'Prize Pool': 'prize', 'Date': 'date', 'Event Date': 'date', 'Notes': 'notes', 'Counted': 'counted',
    'Published': 'published', 'Results': 'results', 'ID': 'id', 'Title': 'title', 'Type': 'type', 'ImageURL': 'imageUrl', 'Link': 'link'
  };
  return map[key] || key;
}
...
function writeSection_(name, items) {
  ...
  let headers = [];
  if (sheet.getLastColumn() > 0 && sheet.getLastRow() > 0) headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getDisplayValues()[0].map(cleanHeader_).filter(Boolean);
  const seen = {};
  headers.forEach(function(h) { seen[h] = true; });
  normalized.forEach(function(item) { Object.keys(item).forEach(function(key) { if (!seen[key]) { headers.push(key); seen[key] = true; } }); });
  if (!headers.length) headers = ['id'];
  if (headers.length > 100) throw new Error(name + ' has too many columns.');
  const rows = normalized.map(function(item) { return headers.map(function(header) { return item[header] == null ? '' : item[header]; }); });
...
/** Creates the required tabs without deleting existing data. */
...
/**
 * Backwards-compatible setup function for the previous TNFFM Apps Script.
 * Existing saved Apps Script editor selections can still run setupTNFFM().
 */
function setupTNFFM() {
  setupSheets();
}
