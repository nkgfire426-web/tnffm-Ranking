...
async function fetchSheetPayload(): Promise<any | null> { const webhookUrl = process.env.GOOGLE_SHEETS_WEBHOOK_URL; if (!webhookUrl) { console.error("Google Sheets read error: GOOGLE_SHEETS_WEBHOOK_URL is not configured"); return null; } let lastError: unknown = null; for (let attempt = 1; attempt <= SHEET_RETRIES; attempt += 1) { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), SHEET_TIMEOUT_MS); try { const response = await fetch(webhookUrl, { method: "GET", next: { revalidate: 15 }, signal: controller.signal, headers: { Accept: "application/json" } }); if (!response.ok) throw new Error(`Google Apps Script returned HTTP ${response.status}`); const payload = await response.json(); if (!payload || payload.ok === false) throw new Error(String(payload?.message || "Google Apps Script returned an unsuccessful response")); return payload; } catch (error) { lastError = error; if (attempt < SHEET_RETRIES) await new Promise((resolve) => setTimeout(resolve, 350 * attempt)); } finally { clearTimeout(timeout); } } console.error("Google Sheets read error after retries:", lastError); return null; }
