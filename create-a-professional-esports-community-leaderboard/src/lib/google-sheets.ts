...
async function fetchFromLocalFile(): Promise<RawTeam[] | null> {
  try {
    ...
    return Array.isArray(teams) ? teams.map((t) => normalizeTeam(t as any)) : null;
  } catch {
    return null;
  }
}
...
async function fetchSheetPayload(): Promise<any | null> {
  ...
  for (let attempt = 1; attempt <= SHEET_RETRIES; attempt += 1) {
    ...
    try {
      ...
      // An explicit successful response with an empty teams array is a VALID
      // dataset (for example after an admin removes the final team). Do not
      // replace it with sample data or an old local dataset.
...
async function fetchFromGoogleAppsScript(): Promise<RawTeam[] | null> {
  ...
  if (!Array.isArray(teams)) {
    console.error("Google Sheets read error: response did not contain a teams array");
    return null;
  }
