...
async function fetchEventsFromGoogleSheets(): Promise<TrackedEvent[] | null> {
  const cachedPayload = getCachedSheetPayload(); const cachedEvents = normalizeEvents(cachedPayload); if (cachedEvents !== null) return cachedEvents;
  const existingRequest = getSheetReadInFlight(); if (existingRequest) return normalizeEvents(await existingRequest);
  const rawUrl = process.env.GOOGLE_SHEETS_WEBHOOK_URL?.trim(); if (!rawUrl) { console.error("Tracked events: GOOGLE_SHEETS_WEBHOOK_URL is not configured"); return null; }
  let url: URL; try { url = new URL(rawUrl); } catch { console.error("Tracked events: GOOGLE_SHEETS_WEBHOOK_URL is invalid"); return null; }
  if (!/^https:\/\/script\.google\.com\/macros\/s\/[^/]+\/exec\/?$/i.test(url.origin + url.pathname)) { console.error("Tracked events: GOOGLE_SHEETS_WEBHOOK_URL must point to the deployed /exec web app"); return null; }
  const request = (async () => { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), SHEET_TIMEOUT_MS); try { url.searchParams.set("_tnffm_events", `${Date.now()}-${Math.random().toString(36).slice(2)}`); const response = await fetch(url.toString(), { method: "GET", cache: "no-store", headers: { Accept: "application/json", "Cache-Control": "no-cache, no-store, max-age=0", Pragma: "no-cache" }, signal: controller.signal }); if (!response.ok) { console.error(`Tracked events Google Sheets read failed: HTTP ${response.status}`); return null; } const payload = await response.json(); if (!payload || payload.ok === false) { console.error(`Tracked events Google Sheets returned an unsuccessful response: ${String(payload?.message || payload?.error || "unknown error")}`); return null; } setCachedSheetPayload(payload); return payload; } catch (error) { console.error("Google Sheets events read error:", error); return null; } finally { clearTimeout(timeout); } })().finally(() => setSheetReadInFlight(null));
