...
function eventCounts(event: TrackedEvent) {
  return event.published === true && event.results && event.results.length > 0 && prizeValue(event.prize) > 1000;
}
...
export function rankTeams(teams: RawTeam[], events: TrackedEvent[] = []): RankedTeam[] {
  const calculatedTeams = events.length ? buildTeamsFromPublishedResults(teams, events) : teams;
