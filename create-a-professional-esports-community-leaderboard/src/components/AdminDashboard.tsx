...
export function AdminDashboard({ initialTeams, initialEvents, initialCollaborators }: { initialTeams: RawTeam[]; initialEvents: TrackedEvent[]; initialCollaborators?: Collaborator[] }) {
  ...
  async function saveChanges() {
    ...
    try {
      ...
      const normalizedTeams = teams.map((team) => { const copy: any = { ...team }; numericKeys.forEach((key) => { if (key in copy) copy[key] = typeof copy[key] === "number" ? copy[key] : Number(String(copy[key] || "").trim()) || 0; }); copy.eventsPlayed = getEventsPlayed(copy); copy.officialMatchFinalists = Math.max(0, Number(copy.officialMatchFinalists) || 0); return copy; });
