...
export function AdminDashboard({ initialTeams, initialEvents, initialCollaborators }: { initialTeams: RawTeam[]; initialEvents: TrackedEvent[]; initialCollaborators?: Collaborator[] }) {
  ...
  async function refreshFromSheet(showMessage = true) {
    ...
    try {
      ...
      setSaveStatus(error instanceof Error ? error.message : "Google Sheets refresh failed.");
  ...
  function updateTeam(index: number, key: keyof RawTeam | "badge", value: string) {
    setTeams((current) => current.map((team, i) => {
      if (i !== index) return team;
      return { ...team, [key]: STRING_TEAM_FIELDS.has(String(key)) ? value : Number(value) } as RawTeam;
    }));
  }
  ...
  function addTeam() { setTeams((current) => [...current, { teamName: "New Team", badge: "", logoUrl: "", kills: 0, booyahs: 0, championships: 0, runnerUp: 0, secondRunnerUp: 0, top5Finishes: 0, finalistFinishes: 0, officialMatchFinalists: 0, eventsPlayed: 0, grandFinals: 0, winRate: 0, killRatio: 0, players: 5, roster: [], status: "Active" } as RawTeam]); setActiveTab("teams"); }
  ...
  function addCollaborator() { setCollaborators((current) => [...current, { name: "New Collaborator", role: "Partner", logoUrl: "", url: "" }]); setActiveTab("collaborators"); setSaveStatus("New collaborator added. Fill the fields and press Save Changes."); }
  ...
  function updateRosterPlayer(teamIndex: number, playerIndex: number, key: "name" | "uid", value: string) { setTeams((current) => current.map((team, i) => i === teamIndex ? { ...team, roster: (team.roster || []).map((player, pi) => pi === playerIndex ? { ...player, [key]: value } : player) } : team)); }
  ...
  async function saveChanges() {
    ...
    try {
      ...
      setSaveStatus("✓ Teams, badges, logos, events and collaborators saved to Google Sheets.");
      // Read back from the source of truth so the admin screen and homepage use exactly the saved values.
