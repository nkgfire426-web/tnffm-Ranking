...
export function AdminDashboard({ initialTeams, initialEvents, initialCollaborators }: { initialTeams: RawTeam[]; initialEvents: TrackedEvent[]; initialCollaborators?: Collaborator[] }) {
  ...
  async function saveChanges() {
    ...
    try {
      ...
      const normalizedTeams = teams.map((team) => { const copy: any = { ...team }; numericKeys.forEach((key) => { if (key in copy) copy[key] = typeof copy[key] === "number" ? copy[key] : Number(String(copy[key] || "").trim()) || 0; }); const manualEventsPlayed = Number(copy.eventsPlayed); copy.eventsPlayed = Number.isFinite(manualEventsPlayed) && manualEventsPlayed >= 0 ? manualEventsPlayed : getEventsPlayed(copy); copy.officialMatchFinalists = Math.max(0, Number(copy.officialMatchFinalists) || 0); return copy; });
...
const Field = memo(function Field({ label, value, onChange, type = "text" }: { label: string; value: string | number | null | undefined; onChange: (value: string) => void; type?: string }) {
  ...
  return <label className="block"><span className="mb-1 block text-xs font-bold uppercase tracking-wider text-slate-400">{label}</span><input ref={inputRef} type={type} value={localValue} onChange={(e) => handleChange(e.target.value)} className="w-full rounded-lg border border-white/10 bg-black/45 px-3 py-2.5 text-white outline-none transition focus:border-gold/60" /></label>;
});
