...
export function TeamDetailsClient({ teams }: { teams: RankedTeam[] }) {
  ...
  return <div className="mt-6 grid gap-6 lg:grid-cols-3">
    ...
    <div className="lg:col-span-2"><div className="rounded-2xl border border-white/10 bg-black/20 p-6">{selected ? <article><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><TeamLogo src={selected.logoUrl} name={selected.teamName} size={96} champion={Number(selected.rank) === 1} /><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.25em] text-gold">Tamil Community Team</p><h2 className="mt-1 font-rajdhani text-4xl font-bold uppercase text-white">{selected.teamName}</h2><p className="mt-1 text-sm text-slate-400">Registered team profile{selected.rank > 0 ? ` • TNFFM Community Rank #${selected.rank}` : ""}</p></div></div>{selected.description ? <p className="mt-6 text-sm leading-7 text-slate-300">{selected.description}</p> : null}<div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-gold/20 bg-gold/5 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Rank</p><p className="mt-1 font-rajdhani text-2xl font-bold text-gold">{selected.rank > 0 ? `#${selected.rank}` : "—"}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Community Score</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{Number(selected.communityPoints || 0).toLocaleString("en-IN")}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Events</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{Number(selected.eventsPlayed || 0)}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Players</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{normalizeRoster(selected.roster).length || Number(selected.players || 0)}</p></div></div><div className="mt-6 rounded-2xl border border-gold/20 bg-gradient-to-r from-gold/10 to-red-500/5 p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold uppercase tracking-wide text-gold">Official Ranking Poster</p><p className="mt-1 text-xs leading-5 text-slate-400">Generate this selected team&apos;s current TNFFM ranking details as a 1080×1350 PNG. The poster uses the team&apos;s official ranking data.</p></div><Link href={`/ranking-poster?team=${encodeURIComponent(selected.teamName)}`} className="inline-flex shrink-0 items-center justify-center rounded-xl bg-gold px-5 py-3 text-sm font-black text-black transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold">Download Ranking Poster</Link></div></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Championships</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{Number(selected.championships || 0)}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Runner-Up</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{Number(selected.runnerUp || 0)}</p></div><div className="rounded-xl border border-white/10 bg-white/[0.03] p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Top 5 Finishes</p><p className="mt-1 font-rajdhani text-2xl font-bold text-white">{Number(selected.top5Finishes || 0)}</p></div></div><div className="mt-8"><div className="flex items-center justify-between gap-3"><h3 className="font-rajdhani text-2xl font-bold uppercase text-white">Team Roster</h3>{loadingRoster && <span className="text-xs text-slate-400">Loading latest players…</span>}</div>{rosterError && <p className="mt-2 text-sm text-amber-300">{rosterError}</p>}{normalizeRoster(selected.roster).length > 0 ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{normalizeRoster(selected.roster).map((player, index) => <div key={`${player.uid || "player"}-${index}`} className="rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3"><p className="font-semibold text-white">{player.name || "Player"}</p>{player.uid ? <p className="mt-1 text-xs text-slate-500">UID: {player.uid}</p> : <p className="mt-1 text-xs text-slate-600">UID not provided</p>}</div>)}</div> : <div className="mt-3 rounded-lg border border-dashed border-white/10 bg-white/[0.02] px-4 py-5 text-sm text-slate-500">No player details have been registered for this team yet.</div>}</div></article> : <div className="p-8 text-center text-slate-400">Select a registered team to view its community profile and ranking data.</div>}</div></div>
  </div>;
}
