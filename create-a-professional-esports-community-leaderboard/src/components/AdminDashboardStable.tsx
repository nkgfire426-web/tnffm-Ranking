...
type SheetPayload = { ok?: boolean; message?: string; teams?: RawTeam[]; teamDetails?: RawTeam[]; rankings?: any[]; events?: TrackedEvent[]; collaborators?: Collaborator[]; news?: any[]; counts?: Record<string, number> };
...
export function AdminDashboardStable({ initialTeams, initialEvents, initialCollaborators }: { initialTeams: RawTeam[]; initialEvents: TrackedEvent[]; initialCollaborators?: Collaborator[] }) {
  ...
  async function refresh(show = true) {
    ...
    try {
      ...
      const syncedTeams = Array.isArray(result.teamDetails) ? result.teamDetails : result.teams;
      if (Array.isArray(syncedTeams)) setTeams(wrapTeams(syncedTeams));
      ...
      if (show) {
        ...
        setStatus(`Google Sheets synced — ${syncedTeams?.length || 0} teams, ${result.events?.length || 0} events, ${result.rankings?.length || counts.rankings || 0} official rankings, ${counts.rosters || 0} roster players.`);
      }
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    {tab === "teams" && <div className="space-y-4">{teams.length === 0 && <div className="glass rounded-xl p-8 text-center text-slate-400">No registered teams yet.</div>}{teams.map((team) => { const ranking = team as any; return <div key={team._id} className="glass rounded-xl p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-start"><TeamLogo src={team.logoUrl} name={team.teamName} size={72} /><div className="grid min-w-0 flex-1 grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3"><Field label="Team Name" value={team.teamName} onChange={(v) => updateTeam(team._id, "teamName", v)} /><Field label="Logo URL" value={team.logoUrl} onChange={(v) => updateTeam(team._id, "logoUrl", v)} /><Field label="Banner URL" value={team.bannerUrl || ""} onChange={(v) => updateTeam(team._id, "bannerUrl", v)} /><Field label="Description" value={team.description || ""} onChange={(v) => updateTeam(team._id, "description", v)} /><Field label="Mobile Number" value={team.mobileNumber || ""} onChange={(v) => updateTeam(team._id, "mobileNumber", v)} inputMode="tel" /><Field label="Players" value={team.players ?? ""} onChange={(v) => updateTeam(team._id, "players", v)} inputMode="numeric" /><Field label="Status" value={team.status || "Active"} onChange={(v) => updateTeam(team._id, "status", v)} /><Field label="Registration Status" value={team.registrationStatus || "Registered"} onChange={(v) => updateTeam(team._id, "registrationStatus", v)} /></div><button onClick={() => setRosterId(team._id)} className="shrink-0 rounded-lg border border-gold/30 px-3 py-2 text-sm font-bold text-gold">Roster</button></div><div className="mt-4 grid grid-cols-2 gap-2 border-t border-white/10 pt-4 sm:grid-cols-4 lg:grid-cols-8">{[["Rank", ranking.rank || 0],["Community Score", ranking.communityScore || 0],["Championships", ranking.championships || 0],["Runner-Up", ranking.runnerUp || 0],["2nd Runner-Up", ranking.secondRunnerUp || 0],["Top 5", ranking.top5Finishes || 0],["Events", ranking.eventsPlayed || 0],["Total Points", ranking.totalPoints || 0]].map(([label,value]) => <div key={String(label)} className="rounded-lg border border-white/10 bg-black/20 p-2"><div className="text-[9px] font-bold uppercase tracking-wider text-slate-500">{label}</div><div className="mt-1 text-lg font-black text-white">{String(value)}</div></div>)}</div></div>; })}</div>}
