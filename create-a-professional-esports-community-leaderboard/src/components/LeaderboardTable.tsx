...
export function LeaderboardTable({ teams }: { teams: RankedTeam[] }) {
  ...
  return <section id="leaderboard" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
    <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="font-rajdhani text-sm font-bold uppercase tracking-[0.25em] text-gold">Official Leaderboard</p><h2 className="font-rajdhani text-4xl font-bold uppercase text-white">Community Rankings</h2><p className="mt-1 text-xs text-slate-500">Published official ranking data from the Community Rankings sheet, with team profile details hydrated from the registered-team sheets.</p></div><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><label className="relative min-w-64"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => { setPage(1); setQuery(event.target.value); }} placeholder="Search team" className="w-full rounded-lg border border-white/10 bg-black/45 py-3 pl-10 pr-4 text-sm text-white outline-none transition focus:border-gold/60" /></label><select value={rankFilter} onChange={(event) => { setPage(1); setRankFilter(event.target.value); }} className="rounded-lg border border-white/10 bg-black/45 px-4 py-3 text-sm text-white outline-none transition focus:border-gold/60"><option value="all">All ranks</option><option value="top5">Top 5</option><option value="top10">Top 10</option><option value="champions">Champions only</option></select><select value={statusFilter} onChange={(event) => { setPage(1); setStatusFilter(event.target.value); }} className="rounded-lg border border-white/10 bg-black/45 px-4 py-3 text-sm text-white outline-none transition focus:border-gold/60"><option value="all">All status</option><option value="active">Active</option><option value="inactive">Inactive</option></select><button onClick={exportPng} className="inline-flex items-center justify-center gap-2 rounded-lg bg-gold px-4 py-3 text-sm font-bold text-black transition hover:bg-yellow-300"><Download className="h-4 w-4" />Export PNG</button></div></div>
