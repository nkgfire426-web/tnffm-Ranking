...
export function LeaderboardTable({ teams }: { teams: RankedTeam[] }) {
  // The official ranking page deliberately uses only the server-provided
  // published Community Rankings snapshot. Do not replace it with /api/teams:
  // that endpoint is also used for public team profiles and can contain
  // registered/unranked teams. Admin saves trigger server cache invalidation,
  // so the next navigation/request receives the fresh published ranking.
  ...
  const pageTeams = filtered.slice((safePage - 1) * pageSize, safePage * pageSize);
  ...
  return <section id="leaderboard" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
    <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="font-rajdhani text-sm font-bold uppercase tracking-[0.25em] text-gold">Official Leaderboard</p><h2 className="font-rajdhani text-4xl font-bold uppercase text-white">Community Rankings</h2><p className="mt-1 text-xs text-slate-500">Published official ranking data. Team profile details are hydrated separately from the registered-team sheet.</p></div><div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap"><label className="relative min-w-64"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => { setPage(1); setQuery(event.target.value); }} placeholder="Search team" className="w-full rounded-lg border border-white/10 bg-black/45 py-3 pl-10 pr-4 text-sm text-white outline-none transition focus:border-gold/60" /></label><select value={rankFilter} onChange={(event) => { setPage(1); setRankFilter(event.target.value); }} className="rounded-lg border border-white/10 bg-black/45 px-4 py-3 text-sm text-white outline-none transition focus:border-gold/60"><option value="all">All ranks</option><option value="top5">Top 5</option><option value="top10">Top 10</option><option value="champions">Champions only</option></select><select value={statusFilter} onChange={(event) => { setPage(1); setStatusFilter(event.target.value); }} className="rounded-lg border border-white/10 bg-black/45 px-4 py-3 text-sm text-white outline-none transition focus:border-gold/60"><option value="all">All status</option><option value="active">Active</option><option value="inactive">Inactive</option></select><button onClick={exportPng} className="inline-flex items-center justify-center gap-2 rounded-lg bg-gold px-4 py-3 text-sm font-bold text-black transition hover:bg-yellow-300"><Download className="h-4 w-4" />Export PNG</button></div></div>
    ...
    <div className="mt-5 flex items-center justify-between text-sm text-slate-400"><span>Showing {filtered.length ? (safePage - 1) * pageSize + 1 : 0}-{Math.min(safePage * pageSize, filtered.length)} of {filtered.length}</span><div className="flex gap-2"><button disabled={safePage === 1} onClick={() => setPage((current) => current - 1)} className="rounded-lg border border-white/10 px-4 py-2 disabled:cursor-not-allowed disabled:opacity-40">Previous</button><button disabled={safePage === totalPages} onClick={() => setPage((current) => current + 1)} className="rounded-lg border border-white/10 px-4 py-2 disabled:cursor-not-allowed disabled:opacity-40">Next</button></div></div>
  </section>;
}
