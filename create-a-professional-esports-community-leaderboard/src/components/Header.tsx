...
import { CalendarDays, Crown, ListChecks, LogIn, Megaphone, MoreVertical, Newspaper, ScrollText, Trophy, Users, X } from "lucide-react";
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-black/70 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:px-6 sm:py-4 lg:px-8" aria-label="Primary navigation">
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-2">
            ...
            {team && <Link href="/team-dashboard" className="group flex max-w-[58px] items-center justify-center rounded-xl border border-gold/40 bg-gradient-to-br from-gold/15 via-black/80 to-black/60 p-1.5 shadow-[0_0_18px_rgba(212,175,55,0.12)] transition duration-200 hover:border-gold hover:shadow-[0_0_24px_rgba(212,175,55,0.25)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold active:scale-95 sm:max-w-[210px] sm:justify-start sm:gap-2 sm:rounded-lg sm:p-1.5 sm:px-3 sm:py-2" title={`Open ${team.teamName || "Team"} Dashboard`}><span className="relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full border-2 border-gold/70 bg-black shadow-[0_0_12px_rgba(212,175,55,0.18)] ring-2 ring-gold/10 transition group-hover:ring-gold/30 sm:h-9 sm:w-9 sm:border sm:ring-0"><TeamLogo src={team.logoUrl} name={team.teamName || "Team"} size={38} /><span className="pointer-events-none absolute inset-0 rounded-full bg-gradient-to-br from-white/15 via-transparent to-transparent" /></span><span className="hidden min-w-0 text-left sm:block"><span className="block truncate text-xs text-slate-400">Team</span><span className="block truncate text-sm font-semibold text-white">{team.teamName}</span></span></Link>}
          </div>
        </nav>
      </header>
