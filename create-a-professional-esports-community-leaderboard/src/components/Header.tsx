...
import { CalendarDays, Crown, ListChecks, LogIn, MoreVertical, Newspaper, ScrollText, Trophy, Users, X } from "lucide-react";
          ...
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            ...
            <Link href="/" className="flex min-w-0 items-center gap-2 sm:gap-3" aria-label="TNFFM home">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-gold/40 bg-black/70 p-1 sm:h-11 sm:w-11">
                <Image src="/brand/tnffm-logo.png" alt="TNFFM Esports logo" width={40} height={40} className="h-full w-full object-contain" priority />
              </span>
        ...
        <aside id="tnffm-navigation-drawer" aria-label="TNFFM navigation menu" className={`absolute left-0 top-0 flex h-full w-[min(86vw,340px)] flex-col border-r border-white/10 bg-[#09090b] shadow-2xl transition-transform duration-300 ease-out ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
          <div className="flex h-20 shrink-0 items-center justify-between border-b border-white/10 px-5">
            <Link href="/" className="flex items-center gap-3" onClick={() => setMenuOpen(false)}>
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-gold p-1"><Image src="/brand/tnffm-logo.png" alt="TNFFM" width={40} height={40} className="h-full w-full object-contain" /></span>
              <span><span className="block font-rajdhani text-xl font-black text-white">TNFFM</span><span className="block text-[9px] font-bold uppercase tracking-[.24em] text-slate-500">Community Menu</span></span>
            </Link>
            <button type="button" onClick={() => setMenuOpen(false)} aria-label="Close navigation" className="grid h-10 w-10 place-items-center rounded-lg border border-white/10 text-slate-300 transition hover:border-gold hover:bg-gold hover:text-black focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"><X className="h-5 w-5" aria-hidden="true" /></button>
          </div>
            ...
            <div className="space-y-1">
              {navigation.map(({ href, icon: Icon, label }) => {
                ...
                return <Link key={href} href={href} onClick={() => setMenuOpen(false)} aria-current={active ? "page" : undefined} className={`flex min-h-12 items-center gap-3 rounded-xl border px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold active:scale-[.99] ${active ? "border-gold/30 bg-gold text-black shadow-[0_0_18px_rgba(212,175,55,0.12)]" : "border-transparent text-slate-300 hover:border-white/10 hover:bg-gold hover:text-black"}`}>
                  <Icon className="h-[18px] w-[18px] shrink-0" aria-hidden="true" /><span className="flex-1">{label}</span>{active && <span className="h-1.5 w-1.5 rounded-full bg-current" aria-hidden="true" />}
                </Link>;
              })}
            </div>
