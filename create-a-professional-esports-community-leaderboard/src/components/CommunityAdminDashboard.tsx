...
export function CommunityAdminDashboard({ initialTeams, initialEvents, initialCollaborators }: { initialTeams: RawTeam[]; initialEvents: TrackedEvent[]; initialCollaborators?: Collaborator[] }) {
  ...
  // Keep text exactly as typed while editing. Numeric conversion is deferred until save,
  // so deleting a value, typing multi-digit numbers, pasting, and correcting values work normally.
  function updateTeam(index: number, key: keyof TeamForm, value: string) {
    setTeams((current) => current.map((team, i) => i === index ? { ...team, [key]: value as any } : team));
  }
  function updateEvent(index: number, key: keyof TrackedEvent, value: string | boolean | number) {
    setEvents((current) => current.map((event, i) => i === index ? { ...event, [key]: value as any } : event));
  }
  function updateResult(eventIndex: number, resultIndex: number, key: keyof EventResult, value: string) {
    setEvents((current) => current.map((event, ei) => {
      ...
      // Do not normalize on every keystroke. Normalizing here turned an empty field
      // immediately into 0/1 and made continuous typing impossible.
      results[resultIndex] = { ...currentResult, [key]: value as any };
  ...
  function addPlayer(index: number) { setTeams((current) => current.map((team, i) => i === index ? { ...team, roster: [...(team.roster || []), { name: "", uid: "", role: "", playerLogoUrl: "" }], players: (team.roster || []).length + 1 } : team)); }
  function updatePlayer(teamIndex: number, playerIndex: number, key: "name" | "uid" | "role" | "playerLogoUrl", value: string) { setTeams((current) => current.map((team, i) => i === teamIndex ? { ...team, roster: (team.roster || []).map((player, pi) => pi === playerIndex ? { ...player, [key]: value } : player) } : team)); }
  ...
  function validateEvents() {
    for (const event of events) {
      ...
      const names = event.results.filter((r) => String(r.teamName || "").trim()).map((r) => String(r.teamName).trim().toLowerCase());
      if (new Set(names).size !== names.length) return `${event.name}: the same team cannot appear twice.`;
      if (event.results.some((r) => Number(r.rank) < 1 || Number(r.rank) > 18)) return `${event.name}: rank must be between 1 and 18.`;
      const ranks = event.results.filter((r) => String(r.teamName || "").trim()).map((r) => Number(r.rank));
  ...
  async function save() {
    ...
    try {
      // Convert numeric drafts only when saving. This preserves normal input behavior while
      // ensuring Google Sheets receives clean numeric values.
      const cleanedTeams = teams.map((team) => {
        const copy: any = { ...team };
        copy.players = Math.max(0, Number(copy.players) || 0);
        ["kills", "booyahs", "championships", "runnerUp", "secondRunnerUp", "grandFinals", "winRate", "killRatio", "booyahRatio"].forEach((key) => {
          if (key in copy) copy[key] = Number.isFinite(Number(copy[key])) ? Number(copy[key]) : 0;
        });
        return copy;
      });
      const cleanedEvents = events.map((event) => ({
        ...event,
        teams: Math.max(0, Number(event.teams) || 0),
        matchesPlayed: Math.max(0, Number(event.matchesPlayed) || 0),
        results: (event.results || [])
          .filter((result) => String(result.teamName || "").trim())
          .map((result) => normalizeResult({
            ...result,
            rank: Number(result.rank) || 1,
            positionPoints: Number(result.positionPoints) || 0,
            kills: Number(result.kills) || 0,
            booyahs: Number(result.booyahs) || 0
          }, Number(event.matchesPlayed) || 0))
      }));
      const cleanedCollaborators = collaborators.filter((item) => item.name || item.role || item.logoUrl || item.url);
      const response = await fetch("/api/admin/save", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password, teams: cleanedTeams, events: cleanedEvents, collaborators: cleanedCollaborators }) });
  ...
  return <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    {rosterIndex !== null && teams[rosterIndex] && <div className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4"><div className="glass max-h-[85vh] w-full max-w-2xl overflow-auto rounded-xl p-5"><div className="mb-4 flex items-center justify-between"><h2 className="font-rajdhani text-2xl font-bold uppercase text-white">{teams[rosterIndex].teamName} Roster</h2><button onClick={() => setRosterIndex(null)} className="rounded-md p-2 text-slate-300"><X className="h-5 w-5" /></button></div>{(teams[rosterIndex].roster || []).map((player, playerIndex) => <div key={playerIndex} className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1.5fr_auto]"><Field label={`Player ${playerIndex + 1}`} value={player.name} onChange={(v) => updatePlayer(rosterIndex, playerIndex, "name", v)} /><Field label="UID" value={player.uid} onChange={(v) => updatePlayer(rosterIndex, playerIndex, "uid", v)} /><Field label="Role" value={player.role || ""} onChange={(v) => updatePlayer(rosterIndex, playerIndex, "role", v)} /><Field label="Player Logo URL" value={player.playerLogoUrl || ""} onChange={(v) => updatePlayer(rosterIndex, playerIndex, "playerLogoUrl", v)} /><button onClick={() => removePlayer(rosterIndex, playerIndex)} className="self-end rounded-lg border border-red-500/30 p-3 text-red-300"><Trash2 className="h-4 w-4" /></button></div>)}<button onClick={() => addPlayer(rosterIndex)} className="mt-2 rounded-lg bg-gold px-4 py-2 font-bold text-black"><Plus className="mr-1 inline h-4 w-4" />Add Player</button></div></div>}
  </section>;
}
