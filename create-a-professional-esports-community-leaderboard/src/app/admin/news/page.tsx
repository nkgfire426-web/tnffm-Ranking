...
const emptyNews = (): NewsItem => ({ id: `NEWS-${Date.now()}`, title: "", description: "", date: new Date().toISOString().slice(0, 10), type: "Tournament", status: "Published", imageUrl: "", link: "" });
...
export default function AdminNewsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems(Array.isArray(result.news) ? result.news.map((item: NewsItem) => ({ ...item, imageUrl: item.imageUrl || "", id: item.id || `NEWS-${Date.now()}-${Math.random()}` })) : []);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <article key={item.id} className="rounded-2xl border border-white/10 bg-black/30 p-5">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="min-w-0 flex-1">
                {item.imageUrl && <img src={normalizeImageUrl(item.imageUrl)} alt="" className="mb-4 h-40 w-full max-w-xl rounded-xl border border-white/10 object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} />}
                <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wider"><span className="rounded-full border border-gold/30 px-2 py-1 text-gold">{item.type || "Update"}</span><span className={`rounded-full border px-2 py-1 ${item.status.toLowerCase() === "published" ? "border-emerald-400/30 text-emerald-300" : "border-slate-500/30 text-slate-400"}`}>{item.status}</span><span className="text-slate-500">{item.date}</span></div><h2 className="mt-3 text-xl font-bold text-white">{item.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-400">{item.description}</p>{item.link && <a href={item.link} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-bold text-gold hover:text-white">Open linked page →</a>}
              </div>
      ...
      {editing && <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-4 backdrop-blur-sm"><div className="w-full max-w-2xl rounded-2xl border border-gold/20 bg-[#09090b] p-6 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-gold">News Editor</p><h2 className="font-rajdhani text-3xl font-bold uppercase text-white">{items.some(item => item.id === editing.id) ? "Edit News" : "Add News"}</h2></div><button onClick={() => setEditing(null)} className="rounded-lg p-2 text-slate-400 hover:text-white"><X className="h-5 w-5" /></button></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="md:col-span-2 text-sm text-slate-300">Title<input value={editing.title} onChange={e => setEditing({ ...editing, title: e.target.value })} className="mt-2 w-full rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50" placeholder="Tournament announcement" /></label><label className="text-sm text-slate-300">Date<input type="date" value={editing.date} onChange={e => setEditing({ ...editing, date: e.target.value })} className="mt-2 w-full rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50" /></label><label className="text-sm text-slate-300">Type<select value={editing.type} onChange={e => setEditing({ ...editing, type: e.target.value })} className="mt-2 w-full rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50"><option>Tournament</option><option>Announcement</option><option>Update</option><option>Result</option></select></label><label className="md:col-span-2 text-sm text-slate-300">Description<textarea rows={5} value={editing.description} onChange={e => setEditing({ ...editing, description: e.target.value })} className="mt-2 w-full resize-none rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50" placeholder="Write the update shown on the homepage..." /></label><label className="md:col-span-2 text-sm text-slate-300">Image URL<input value={editing.imageUrl} onChange={e => setEditing({ ...editing, imageUrl: e.target.value })} className="mt-2 w-full rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50" placeholder="https://.../tournament-poster.jpg or Google Drive sharing link" /><span className="mt-1 block text-xs text-slate-500">Google Drive sharing links are supported. Use Share → Anyone with the link → Viewer.</span></label><label className="md:col-span-2 text-sm text-slate-300">Optional Link<input value={editing.link} onChange={e => setEditing({ ...editing, link: e.target.value })} className="mt-2 w-full rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-white outline-none focus:border-gold/50" placeholder="https://..." /></label></div><div className="mt-6 flex justify-end gap-2"><button onClick={() => setEditing(null)} className="rounded-lg border border-white/10 px-4 py-3 font-bold text-slate-300">Cancel</button><button onClick={saveEditor} className="rounded-lg bg-gold px-5 py-3 font-bold text-black">Save News</button></div></div></div>}
    </main>
  );
}
