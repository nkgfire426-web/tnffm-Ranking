...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(webhook, { method: "POST", headers: { "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ action: "loginTeam", username, passwordHash: hashPassword(password) }) });
