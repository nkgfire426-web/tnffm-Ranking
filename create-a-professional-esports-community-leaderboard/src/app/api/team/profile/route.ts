...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(webhook, { method: "POST", headers: { "Content-Type": "application/json" }, cache: "no-store", body: JSON.stringify({ action: "updateTeamProfile", username: session.username, teamSlug: session.teamSlug, logoUrl: body.logoUrl, description: body.description, roster: Array.isArray(body.roster) ? body.roster : [] }) });
