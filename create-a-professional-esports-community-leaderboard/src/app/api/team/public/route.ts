  ...
  try {
    ...
    return NextResponse.json({
      ...
      team: {
        ...
        roster: Array.isArray((team as any).roster) ? (team as any).roster : [],
        players: team.players ?? 0,
        status: team.status || "Active",
        communityPoints: team.communityPoints ?? 0,
        championships: team.championships ?? 0,
        runnerUp: (team as any).runnerUp ?? 0,
        secondRunnerUp: (team as any).secondRunnerUp ?? 0,
        rank: (team as any).rank ?? 0,
        kills: (team as any).kills ?? 0,
        booyahs: (team as any).booyahs ?? 0,
        eventsPlayed: (team as any).eventsPlayed ?? 0,
      }
    });
  } catch (error) {
