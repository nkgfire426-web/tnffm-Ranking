...
function points(t: any) {
  return num(t.championships) * 100 + num(t.runnerUp) * 70 + num(t.secondRunnerUp) * 50 + num(t.top5Finishes) * 25 + num(t.finalistFinishes || t.grandFinals) * 15 + num(t.officialMatchFinalists) * 100 + num(t.approvedSubmissionPoints);
}
function normalizeTeams(items: unknown[]) { return items.map((x: any) => ({ ...x, communityPoints: points(x) })); }
function cleanResult(r: any, e: any) {
  const kills = Math.max(0, num(r?.kills)), booyahs = Math.max(0, num(r?.booyahs)), pp = Math.max(0, num(r?.positionPoints));
  const rank = Math.max(1, num(r?.rank ?? r?.position) || 1), matches = Math.max(0, num(e?.matchesPlayed));
  return { resultId: text(r?.resultId ?? r?.id), teamId: text(r?.teamId), teamName: text(r?.teamName ?? r?.team), position: rank, rank, kills, booyahs, positionPoints: pp, killPoints: kills, totalPoints: pp + kills, total: pp + kills, killRatio: matches ? kills / matches : 0, booyahRatio: matches ? booyahs / matches * 100 : 0, proofUrl: text(r?.proofUrl ?? r?.proofURL), verified: bool(r?.verified) };
}
...
function validatePublishedEvents(events: any[], teams: any[]) {
  ...
  for (const event of events) {
    ...
    const name = text(event.name) || "Event", results = Array.isArray(event.results) ? event.results : [];
...
async function readSheet(url: string) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const readUrl = new URL(url); readUrl.searchParams.set("_tnffm_verify", `${Date.now()}-${attempt}-${Math.random().toString(36).slice(2)}`);
...
function verifyList(expected: unknown[], actual: unknown[], name: string, key: (x: any) => string) {
  const expectedList = Array.isArray(expected) ? expected : [], actualList = Array.isArray(actual) ? actual : [];
  ...
  const actualSet = new Set(actualList.map(key));
  const missing = expectedList.map(key).filter(k => !actualSet.has(k));
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (hasEvents) { data.events = cleanEvents(p.events); validatePublishedEvents(data.events, data.teams || []); data.rankingResults = rankingResults(data.events); data.results = data.rankingResults; const publishedEvents = data.events.filter((e: any) => bool(e.published)); if (hasTeams && publishedEvents.length) data.rankings = rankTeams(data.teams as any, publishedEvents as any).map((t: any) => ({ ...t, teamId: t.teamId || data.teams.find((x: any) => text(x.teamName).toLowerCase() === text(t.teamName).toLowerCase())?.teamId || "" })); else if (hasTeams) data.rankings = []; }
    ...
    if (hasEvents && savedResults.length !== data.rankingResults.length) { saved = await readSheet(url); savedResults = Array.isArray(saved.rankingResults) ? saved.rankingResults : Array.isArray(saved.results) ? saved.results : []; }
    ...
    if (data.rankings) { verifyList(data.rankings, saved.rankings || [], "Community Rankings", rankingIdentity); const savedByIdentity = new Map((saved.rankings || []).map((x: any) => [rankingIdentity(x), rankingSnapshot(x)])); for (const expected of data.rankings) { const id = rankingIdentity(expected), actual = savedByIdentity.get(id); if (!actual || actual !== rankingSnapshot(expected)) throw new Error(`Community Rankings was written but read-back statistics do not match for ${text(expected.teamName)}.`); } }
    if (hasCollabs) {
