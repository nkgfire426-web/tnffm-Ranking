...
export default function TeamLoginPage() {
  ...
  return <main className="min-h-screen bg-[#050507] px-4 py-16 text-white"><div className="mx-auto max-w-md"><div className="mb-8 text-center"><p className="font-rajdhani text-sm font-bold uppercase tracking-[0.25em] text-gold">TNFFM Team Portal</p><h1 className="mt-2 font-rajdhani text-4xl font-bold uppercase">{register ? "Create Team Account" : "Team Login"}</h1><p className="mt-3 text-sm text-slate-400">Create an account or login to manage your team profile, logo and roster.</p></div><form onSubmit={submit} className="glass space-y-5 rounded-2xl p-6">
    ...
    <button disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-lg bg-gold px-4 py-3 font-bold text-black disabled:opacity-50">{register ? <UserPlus className="h-4 w-4" /> : <LogIn className="h-4 w-4" />}{loading ? "Please wait..." : register ? "Create Account" : "Login"}</button>
