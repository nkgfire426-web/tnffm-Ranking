 ...
 * Standalone Apps Script. Set Script Property: SPREADSHEET_ID
 * Team signup creates a new team + TeamAccounts record immediately.
 * Passwords are stored as SHA-256 hashes sent by Vercel.
 */
...
const TEAM_HEADERS = [
  ...
  "Status", "Description", "LastUpdated", "Mobile Number"
];
...
function doGet() {
  try {
    ...
    return json_({ ok: true, teams: readTeams_(ss), events: readObjects_(ss, "Events"), collaborators: readObjects_(ss, "Collaborators") });
  } catch (err) {
...
function registerTeam_(ss, body) {
  ...
  if (!passwordHash) return json_({ ok: false, message: "Password is required." });
  ...
  for (let i = 1; i < accountRows.length; i++) {
    if (String(accountRows[i][0] || "").trim().toLowerCase() === username) return json_({ ok: false, message: "Username is already registered." });
  }

  const teams = readTeams_(ss);
  if (teams.some(function (team) { return String(team.teamName || "").trim().toLowerCase() === teamName.toLowerCase(); })) {
  ...
  const team = {
    teamName: teamName, slug: slug, rank: 0, previousRank: 0, communityPoints: 0,
    badge: "", logoUrl: "", bannerUrl: "", kills: 0, booyahs: 0, championships: 0,
    runnerUp: 0, secondRunnerUp: 0, top3Finishes: 0, finalistFinishes: 0,
    officialMatchFinalists: 0, eventsPlayed: 0, grandFinals: 0, winRate: 0,
    killRatio: 0, players: 0, roster: [], status: "Active", description: "",
    lastUpdated: now, mobileNumber: ""
  };

  teams.push(team);
  writeTeams_(ss, teams);
  accounts.appendRow([username, passwordHash, slug, email, "Active", now, now]);
  SpreadsheetApp.flush();
...
function loginTeam_(ss, body) {
  const username = String(body.username || "").trim().toLowerCase();
  const passwordHash = String(body.passwordHash || "").trim();
  if (!username || !passwordHash) return json_({ ok: false, message: "Username and password are required." });
  ...
  for (let i = 1; i < rows.length; i++) {
    const storedUsername = String(rows[i][0] || "").trim().toLowerCase();
    ...
    if (storedUsername === username && storedHash === passwordHash) {
      if (status.toLowerCase() !== "active") return json_({ ok: false, message: "This team account is not active." });
      return json_({ ok: true, username: username, teamSlug: teamSlug, email: email, status: status, message: "Login successful." });
    }
  }
...
function getTeam_(ss, body) {
  ...
  const team = readTeams_(ss).find(function (item) { return item.slug === slug; });
...
function updateTeamProfile_(ss, body) {
  ...
  if (!account || account.teamSlug !== teamSlug) return json_({ ok: false, message: "Team account is not authorized." });

  const teams = readTeams_(ss);
  const team = teams.find(function (item) { return item.slug === teamSlug; });
  if (!team) return json_({ ok: false, message: "Team not found." });
  ...
  if (typeof body.mobileNumber === "string") team.mobileNumber = body.mobileNumber.trim();

  if (Array.isArray(body.roster)) {
    team.roster = body.roster.map(function (player) {
      return { name: String(player.name || "").trim(), uid: String(player.uid || "").trim() };
    }).filter(function (player) { return player.name || player.uid; });
...
function resetTeamPassword_(ss, body) {
  ...
  if (!username || !newHash) return json_({ ok: false, message: "Password reset data is missing." });
  const sheet = ss.getSheetByName("TeamAccounts");
  if (!sheet || sheet.getLastRow() < 2) return json_({ ok: false, message: "Team account not found." });
  const rows = sheet.getDataRange().getValues();
  for (let i = 1; i < rows.length; i++) {
...
function findAccount_(ss, username) {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0] || "").trim().toLowerCase() === normalized) {
      return { username: String(rows[i][0] || ""), teamSlug: String(rows[i][2] || ""), email: String(rows[i][3] || ""), status: String(rows[i][4] || "Active") };
    }
  }
  return null;
}
...
function createUniqueSlug_(teamName, teams) {
  const base = String(teamName || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 45) || "team";
  let slug = base;
  let number = 2;
  while (teams.some(function (team) { return String(team.slug || "").toLowerCase() === slug; })) slug = base + "-" + number++;
  return slug;
}
...
function writeTeams_(ss, teams) {
  ...
  const rows = teams.map(function (team) {
    return [
      team.teamName || "", team.slug || "", value_(team.rank), value_(team.previousRank), value_(team.communityPoints),
      team.badge || "", team.logoUrl || "", team.bannerUrl || "", value_(team.kills), value_(team.booyahs),
      value_(team.championships), value_(team.runnerUp), value_(team.secondRunnerUp), value_(team.top3Finishes),
      value_(team.finalistFinishes), value_(team.officialMatchFinalists), value_(team.eventsPlayed), value_(team.grandFinals),
      value_(team.winRate), value_(team.killRatio), value_(team.players), typeof team.roster === "string" ? team.roster : JSON.stringify(team.roster || []),
      team.status || "Active", team.description || "", team.lastUpdated || "", team.mobileNumber || ""
    ];
  });
...
function readTeams_(ss) {
  ...
  return values.slice(1).filter(function (row) { return String(row[0] || "").trim() !== ""; }).map(function (row) {
    let roster = [];
    try { roster = row[21] ? JSON.parse(String(row[21])) : []; } catch (err) { roster = []; }
    return {
      teamName: String(row[0] || ""), slug: String(row[1] || ""), rank: number_(row[2]), previousRank: number_(row[3]),
      communityPoints: number_(row[4]), badge: String(row[5] || ""), logoUrl: String(row[6] || ""), bannerUrl: String(row[7] || ""),
      kills: number_(row[8]), booyahs: number_(row[9]), championships: number_(row[10]), runnerUp: number_(row[11]),
      secondRunnerUp: number_(row[12]), top3Finishes: number_(row[13]), finalistFinishes: number_(row[14]),
      officialMatchFinalists: number_(row[15]), eventsPlayed: number_(row[16]), grandFinals: number_(row[17]), winRate: number_(row[18]),
      killRatio: number_(row[19]), players: number_(row[20]), roster: roster, status: String(row[22] || "Active"),
      description: String(row[23] || ""), lastUpdated: String(row[24] || ""), mobileNumber: String(row[25] || "")
    };
  });
}
...
function writeObjects_(ss, sheetName, objects) {
  ...
  const headers = [];
  objects.forEach(function (object) { Object.keys(object || {}).forEach(function (key) { if (headers.indexOf(key) === -1) headers.push(key); }); });
  const rows = objects.map(function (object) { return headers.map(function (key) { const value = object[key]; return value === null || value === undefined ? "" : typeof value === "object" ? JSON.stringify(value) : value; }); });
...
function readObjects_(ss, sheetName) {
  ...
  return values.slice(1).filter(function (row) { return row.some(function (value) { return String(value || "").trim() !== ""; }); }).map(function (row) {
    const object = {};
...
function uploadLogo_(body) {
  if (!body.dataUrl || typeof body.dataUrl !== "string") return json_({ ok: false, message: "Logo data is missing." });
  const match = body.dataUrl.match(/^data:([^;]+);base64,(.+)$/);
  if (!match) return json_({ ok: false, message: "Invalid image data." });
  const fileName = String(body.fileName || "tnffm-logo").replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 80);
  const blob = Utilities.newBlob(Utilities.base64Decode(match[2]), match[1], fileName);
  const file = DriveApp.createFile(blob);
  file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
  const id = file.getId();
  return json_({ ok: true, url: "https://drive.google.com/uc?export=view&id=" + encodeURIComponent(id), fileId: id });
}

function getOrCreateSheet_(ss, name) { return ss.getSheetByName(name) || ss.insertSheet(name); }
function ensureHeaders_(sheet, headers) { sheet.getRange(1, 1, 1, headers.length).setValues([headers]); sheet.setFrozenRows(1); }
