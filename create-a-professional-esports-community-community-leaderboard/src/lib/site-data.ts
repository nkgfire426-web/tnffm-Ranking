...
function mergeProfile(ranking: RankedTeam, profile?: RawTeam): RankedTeam {
  ...
  // IMPORTANT: Community Rankings is the sole authority for ranking/stat fields.
  // Registered Teams/Rosters is the sole authority for current public profile fields.
  // Never spread profile after ranking because that could overwrite published ranking data.
  return {
    ...ranking,
...
export async function getUnifiedTeamData(): Promise<RankedTeam[]> {
  ...
  return ranked.map((ranking) => {
    const profile =
      byId.get(text((ranking as any).teamId)) ||
      byName.get(text(ranking.teamName).toLowerCase());
    return mergeProfile(ranking, profile);
  });
}

/** Public team profiles: every registered team, with ranking data added only when published. */
export async function getPublicTeamData(): Promise<RankedTeam[]> {
  ...
  return registered.map((profile) => {
    ...
    // Unranked teams remain complete registered profiles, with ranking values explicitly zeroed.
    return {
      ...profile,
      ...
      championships: numberOr(profile.championships),
      runnerUp: numberOr(profile.runnerUp),
      secondRunnerUp: numberOr(profile.secondRunnerUp),
      top5Finishes: numberOr(profile.top5Finishes),
      finalistFinishes: numberOr(profile.finalistFinishes),
      officialMatchFinalists: numberOr(profile.officialMatchFinalists),
      eventsPlayed: numberOr(profile.eventsPlayed),
      grandFinals: numberOr(profile.grandFinals),
      kills: numberOr(profile.kills),
      booyahs: numberOr(profile.booyahs),
      winRate: numberOr(profile.winRate),
      killRatio: numberOr(profile.killRatio),
      booyahRatio: numberOr(profile.booyahRatio),
      positionPoints: numberOr(profile.positionPoints),
      totalPoints: numberOr(profile.totalPoints),
      matchesPlayed: numberOr(profile.matchesPlayed),
      approvedSubmissionPoints: numberOr(profile.approvedSubmissionPoints),
